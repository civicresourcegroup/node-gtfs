...
/*
* Retuns all route ids
*/
exports.getAllRouteIds = (agency_key, cb) => Route.find({ agency_key }).select('route_id').exec(cb);
...
exports.getRoutesByAgency = (agency_key, agency_id, cb) => {
  ...
  return Route.find(query).select('-_id').exec(cb);
};
 ...
 */
exports.getRoutesById = (agency_key, route_id, cb) => Route.findOne({
  agency_key,
  route_id
}).select('-_id').exec(cb);
