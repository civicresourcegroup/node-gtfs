...
*/
exports.getAllStopIds = (agency_key, cb) => {
  const query = {
    agency_key
  };
  return Stop.find(query).select('stop_id').select('-_id').exec(cb);
};
...
exports.getStops = (agency_key, stop_ids, cb) => {
  ...
  return Stop.find(query).select('-_id').exec(cb);
};


/*
 * Returns an array of stops along the `trip_id` for the `agency_key` specified
 */
 exports.getStopsByTripId = (agency_key, trip_id, cb) => {
    const query = {
      agency_key,
      trip_id
    };
    return StopTime.find(query).sort({stop_sequence: 1}).exec()
      .then(getStops);

    function getStops(stopTimes) {
      ...
      function getSorted(arr, sortArr) {
       ...
       function getByValue(arr, value) {
         for (var i=0, iLen=arr.length; i<iLen; i++) {
           if (arr[i].stop_id == value) return arr[i];
         }
       }

      const stopIds = stopTimes.map(i => i.stop_id);
      return Stop.find({ stop_id: { $in: stopIds } }).select('-_id').exec().then(items =>
        cb(null, getSorted(items,stopIds).filter(i => i)) ,cb);
    }
