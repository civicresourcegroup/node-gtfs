...
/*
* Returns all shape IDs
*/
exports.getAllShapeIds = (agency_key,cb) => {
  const query = {
    agency_key
  };
  return Shape.find(query).distinct('shape_id').exec(cb);
}
...
/*
* Returns an array of shapes for 'trip_id' specified
*/
exports.getShapeByTripId = (agency_key, trip_id, cb) => {
  const query = {
    agency_key,
    trip_id
  }
   Trip.findOne(query).select('shape_id').exec().then(getShape);

   function getShape(trip) {
     return Shape.find({'agency_key': agency_key, 'shape_id':trip.shape_id}).sort({shape_pt_sequence: 1}).select('-_id').exec(cb);
   }
}
...
exports.getShapesByShapeId = (agency_key, shape_ids) => {
  ...
  return Promise.all(shape_ids.map(shape_id => Shape.find({
    agency_key,
    shape_id
  })
